...
def plot_nav_steps(df):
    ...
    plt.title(" User 1 - Average Navigation Steps by Mode")
    plt.ylabel("Nav Steps")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "U1_fig_nav_steps.png")
    plt.close()
...
def plot_fix_quality(df):
    ...
    plt.title("User 1 - Average Fix Quality by Mode")
    plt.ylabel("Fix Score")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "U1_fig_fix_quality.png")
    plt.close()
...
def plot_boxplot(df):
    ...
    plt.title("User 1 - Navigation Steps Distribution")
    plt.suptitle("")
    plt.ylabel("Nav Steps")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "U1_fig_nav_boxplot.png")
    plt.close()
...
def plot_tradeoff(df):
    ...
    plt.ylabel("Fix Score")
    plt.title("User 1 - Navigation vs Fix Quality Tradeoff")
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUT_DIR / "U1_fig_tradeoff.png")
    plt.close()
...
def plot_improvement(df):
    ...
    plt.title(" User 1- Navigation Improvement (Baseline - Tool)")
    plt.xlabel("Improvement")
    plt.ylabel("Frequency")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "U1_fig_improvement_hist.png")
    plt.close()
